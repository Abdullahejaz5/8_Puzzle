...
import numpy as np
#49,153
#simple heuistic method visits less states to give our sol in less time but with more moves
class PriorityQueue:
    def __init__(self):
        self.l=[]
    def enqueue(self, x):
...
class Node:
    ...
    def heuristic(self,state):
        d={1:(0,0),2:(0,1),3:(0,2),4:(1,0),5:(1,1),6:(1,2),7:(2,0),8:(2,1)," ":(2,2)}
        count=0
        for i in range(len(state)):
            for j in range(len(state[i])):
                if d[state[i][j]]!=(i,j):
                    s1=d[state[i][j]]
                    s=abs(i-s1[0])+abs(j-s1[1])
                    count+=s
        return count
    def __str__(self):
        l=""
